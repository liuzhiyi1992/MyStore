...
#import "CourtPoolListDateView.h"
#import "DayBookingInfo.h"
#import "DateUtil.h"
#import "NSDate+Utils.h"

@interface CourtPoolListDateView()

@end

@implementation CourtPoolListDateView

+(CourtPoolListDateView *)createCourtPoolListDateViewWithHoldViewWhite:(CGFloat)holdViewWidth{
    
    NSArray *topLevelObjects = [[NSBundle mainBundle] loadNibNamed:@"CourtPoolListDateView" owner:self options:nil];
    if (topLevelObjects == nil || [topLevelObjects count] <= 0){
        return nil;
    }
    CourtPoolListDateView *view = [topLevelObjects objectAtIndex:0];
//    [view.selectButton setBackgroundImage:[SportColor createImageWithColor:[UIColor clearColor]] forState:UIControlStateNormal];
    ...
    view.frame = CGRectMake(view.frame.origin.x, view.frame.origin.y, holdViewWidth/ 3, view.frame.size.height);
...
- (void)updateSelected:(BOOL)isSelected
{
    ...
    if (isSelected){
        textColor = [UIColor colorWithRed:0.0/255.0 green:159.0/255.0 blue:240.0/255.0 alpha:1];
        bottomColor = textColor;
       
    } else {
        textColor = [UIColor colorWithRed:102.0/255.0 green:102.0/255.0 blue:102.0/255.0 alpha:1];
//        bottomColor = [UIColor colorWithRed:230.0/255.0 green:230.0/255.0 blue:230.0/255.0 alpha:1];
...
- (IBAction)clickButton:(id)sender {
    ...
    if ([_delegate respondsToSelector:@selector(didClickCourtPoolListDateView:)]){
        [_delegate didClickCourtPoolListDateView:_date];
    }
}
