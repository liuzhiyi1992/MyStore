...
@interface OrderConfirmController ()<ConfirmPayTypeCellDelegate>
...
- (void)viewDidLoad
{
    ...
    self.orderHolderView.hidden = YES;
    self.refundTipsHolderView.hidden = YES;
    self.priceHolderView.hidden = YES;
    self.submitHolderView.hidden = YES;
    self.tipsLabel.hidden = YES;
    self.selectedConfirmPayType = ConfirmPayTypeNone;
    
    [(UIImageView *)[self.orderHolderView viewWithTag:200] setImage:[SportImage whiteBackgroundImage]];
    
    for (UIView *first in self.view.subviews) {
        for (UIView *second in first.subviews) {
            [self updateLineImageView:second];
            for (UIView *third in second.subviews) {
                [self updateLineImageView:third];
                for (UIView *fourth in third.subviews) {
                    [self updateLineImageView:fourth];
                }
            }
        }
    }
...
//    [self.submitButton setBackgroundImage:[SportImage orangeButtonImage] forState:UIControlStateNormal];
    ...
    [SportProgressView showWithStatus:@"订单填写中..."];
...
-(void) didConfirmOrder:(NSString *)status msg:(NSString *)msg confirmOrder:(ConfirmOrder *)confirmOrder {
    if ([status isEqualToString:STATUS_SUCCESS]) {
        ...
        self.tipsLabel.text = [NSString stringWithFormat:@"%@,请返回再试", message];
    }
}
...
- (void)updateDataViews
{
    ...
    if (_order.useDate) {
        ...
        self.useDateLabel.text = [NSString stringWithFormat:@"%@ (%@)", weekString, dateString];
    }
    
    self.totalPriceLabel.text = [NSString stringWithFormat:@"%@元", [PriceUtil toValidPriceString:_order.totalAmount]];
    
    [[self.orderBaseHolderView viewWithTag:TAG_PRODUCT_DETAIL_GROUP_VIEW] removeFromSuperview];
    
    ProductDetailGroupView *detailView = [ProductDetailGroupView createProductDetailGroupView];
    detailView.tag = TAG_PRODUCT_DETAIL_GROUP_VIEW;
    
    //v1.97 默认不显示价钱(场次只可能出现 Online，MemberCard，None)
    BOOL showPrice = (self.selectedConfirmPayType == ConfirmPayTypeOnline);
    [detailView updateViewWithOrder:_order showPrice:showPrice];
    [detailView updateOriginX:0];
    [detailView updateOriginY:43];
    
    //水平线条调整
    for (UIImageView *horLineImage in detailView.horiLineImageViews) {
        [horLineImage setImage:[SportImage lineImage]];
        [horLineImage updateHeight:1.f];
    }
    
    //垂直线样式调整
    UIImageView *verLineImage = [detailView viewWithTag:301];
    [verLineImage setImage:[SportImage lineVerticalImage]];
    [verLineImage updateWidth:1.f];
    
    [self.orderBaseHolderView addSubview:detailView];
    
    [self.orderBaseBottomHolderView updateOriginY:detailView.frame.origin.y + detailView.frame.size.height];
    [self.orderBaseHolderView updateHeight:_orderBaseBottomHolderView.frame.origin.y + _orderBaseBottomHolderView.frame.size.height];
    [(UIImageView *)[self.orderBaseHolderView viewWithTag:200] updateHeight:_orderBaseHolderView.frame.size.height];
    [self.orderHolderView updateHeight:_orderBaseHolderView.frame.origin.y + _orderBaseHolderView.frame.size.height];
    y = y + _orderHolderView.frame.size.height + SPACE;
    ...
    if (self.isCardUser) {
        ...
        if(_selectedConfirmPayType == ConfirmPayTypeMembershipCard) {
            [self.confirmPayTypeHolderView updateHeight:[ConfirmPayTypeCell getCellHeight]*2 + [ConfirmPayTypeCell getCellHeight] * [self.cardList count]];
        } else {
            [self.confirmPayTypeHolderView updateHeight:[ConfirmPayTypeCell getCellHeight]*2];
        }
    ...
    if (self.isCardUser == YES && self.selectedConfirmPayType != ConfirmPayTypeOnline) {
        ...
        [self.priceHolderView updateHeight:_orderConfirmPriceView.frame.size.height];
        y = y + _priceHolderView.frame.size.height + SPACE;
        
    }
    
    //设置提交按钮
    [self.submitHolderView updateOriginY:y];
    y = y + _submitHolderView.frame.size.height;
    
    //设置退款提示
    [self.refundTipsHolderView updateOriginY:y];
    y = y + _refundTipsHolderView.frame.size.height + SPACE;
...
- (IBAction)clickSubmitButton:(id)sender {
    ...
    [OrderService submitOrder:self
            ...
            insuranceQuantity:self.orderConfirmPriceView.insuranceNumber
     saleList:nil];
}
...
- (IBAction)clickSelectActivityButton:(id)sender {
    FavourableActivityView *view = [FavourableActivityView createViewWithActivityList:_order.favourableActivityList selectedActivityId:_selectedActivityId];
    view.delegate = self;
    [view show];
}
